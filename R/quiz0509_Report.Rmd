...
pander(addmargins(table(quiz0509$group, quiz0509$Q1, exclude = NULL), margin = 1))
```

##### Randomization (%)

```{r, randomized placebo control approach in percentage, echo = FALSE}
pander(round(prop.table(addmargins(table(quiz0509$group, quiz0509$Q1, exclude = NULL), margin = 1), margin = 1)[4,]*100, digits = 1))
```
...
pander(addmargins(table(quiz0509$group, quiz0509$Q2, exclude = NULL), margin = 1))
```

##### Treatment vs Control (%)

```{r, opposite in percentage, echo = FALSE}
pander(round(prop.table(addmargins(table(quiz0509$group, quiz0509$Q2, exclude = NULL), margin = 1), margin = 1)[4,]*100, digits = 1), caption = "쏘오크 예방주사의 효과")
```
...
merge.tbl <- table(quiz0509$group, quiz0509$Q3.2)
rownames(merge.tbl) <- c("Red(추상적 질문 먼저)", "Black(구체적 질문 먼저)") 
kable(table(quiz0509$group, quiz0509$Q3.2, exclude = NULL), caption = "Wason Selection 1 in red")
```
...
merge.tbl.2 <- table(quiz0509$group, quiz0509$Q4.2)
rownames(merge.tbl.2) <- c("Red(구체적 질문 먼저)", "Black(추상적 질문 먼저)")
kable(table(quiz0509$group, quiz0509$Q4.2, exclude = NULL), caption = "Wason Selection 1 in Black")
```

##### % 비교.  

```{r, Wason Selection 1 in Black in percentage, echo = FALSE}
kable(round(prop.table(merge.tbl.2, margin = 1)*100, digits = 1))
```
